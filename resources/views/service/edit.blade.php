    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 lg:p-8">
                <form action="{{ route('service.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="w-full">
                    @csrf
                    @method('put')
                    <div class="mt-5">
                        ...
                        <input type="text" placeholder="name" value="{{ old('name')  ?? $item->name }} " name="name" class="w-full
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="mt-5">
                            ...
                            <input type="text" placeholder="tag" value="{{ old('tag')  ?? $item->tag }}" name="tag" class="w-full bg-gray-200 text-sm rounded-md">
                        </div>
                        <div class="mt-5">
                            <label for="imagePath" class="text-gray-700 text-xs font-bold mb-2">
                                Image_path
                            </label>
                            <input type="file"  value="{{ old('imagePath')  ?? $item->imagePath }}" name="imagePath" class="w-full
